...
// Refuse to run outside dev (2.7)
if (process.env.NODE_ENV === 'production') {
    console.error('[FATAL] Seed script refused to run in production');
    process.exit(1);
}
if (process.env.CONFIRM_DESTRUCTIVE_SEED !== 'yes') {
    console.error('[FATAL] Set CONFIRM_DESTRUCTIVE_SEED=yes to acknowledge data loss');
    process.exit(1);
}
const SEED_PASSWORD = process.env.SEED_PASSWORD || 'seed-' + crypto.randomBytes(6).toString('hex');

async function seed() {
    try {
        console.log('🌱 Connecting to MongoDB for seeding...');
        await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/rentoquick');
        ...
        const users = await User.create([
            {
                name: 'Arjun Sharma', email: 'arjun@example.com', password: SEED_PASSWORD,
                phone: '9876543210', bio: 'Hobbyist photographer & tech enthusiast.',
                location: { city: 'Mumbai', state: 'Maharashtra', country: 'India' },
                isVerified: true, isEmailVerified: true, rating: { average: 4.8, count: 12 },
            },
            {
                name: 'Priya Patel', email: 'priya@example.com', password: SEED_PASSWORD,
                phone: '9876501234', bio: 'Outdoor adventure lover & fitness coach.',
                location: { city: 'Bangalore', state: 'Karnataka', country: 'India' },
                isVerified: true, isEmailVerified: true, rating: { average: 4.9, count: 8 },
            },
            {
                name: 'Rahul Gupta', email: 'rahul@example.com', password: SEED_PASSWORD,
                phone: '9811234567', bio: 'DIY enthusiast and tools collector.',
                location: { city: 'Delhi', state: 'Delhi', country: 'India' },
                isVerified: true, isEmailVerified: true, rating: { average: 4.7, count: 15 },
            }
        ]);
